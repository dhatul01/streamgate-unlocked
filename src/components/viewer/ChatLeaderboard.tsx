  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        exit={{ opacity: 0, y: -10 }}
        className="absolute inset-x-0 top-12 z-30 mx-3 rounded-xl border border-primary/30 bg-card/95 p-3 shadow-xl backdrop-blur-sm"
      >
        ...
        {loading ? (
          ...
          <div className="space-y-1">
            {leaders.map((entry, i) => (
              <div key={entry.username} className={`flex items-center gap-2 rounded-lg px-2 py-1.5 ${i < 3 ? "bg-warning/5" : ""}`}>
                <span className="w-5 text-center text-xs font-bold">
                  {i < 3 ? MEDALS[i] : `${i + 1}`}
                </span>
                <span className={`flex-1 truncate text-xs font-medium ${i === 0 ? "text-warning" : "text-foreground"}`}>
                  {entry.username}
                </span>
