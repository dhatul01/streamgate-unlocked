...
const InstallBanner = forwardRef<HTMLDivElement>((_, ref) => {
  ...
  useEffect(() => {
    ...
    // For iOS (no beforeinstallprompt), show after 3s
    ...
    <div className="fixed inset-x-0 bottom-0 z-[90] p-3 animate-in slide-in-from-bottom duration-300">
      <div className="mx-auto max-w-md rounded-2xl border border-border bg-card/95 p-4 shadow-xl backdrop-blur-md">
        <div className="flex items-start gap-3">
          <img src={logo} alt="" className="h-12 w-12 shrink-0 rounded-xl border border-primary/30 shadow-[0_0_12px_hsl(var(--primary)/0.2)]" />
        ...
        <div className="mt-3 flex gap-2">
          ...
          <button
            onClick={dismiss}
            className="rounded-xl bg-secondary px-4 py-2.5 text-sm font-medium text-secondary-foreground transition hover:bg-secondary/80"
          >
            Nanti
          </button>
        </div>
