...
const UserManager = () => {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      const [balancesRes, ordersRes, tokensRes] = await Promise.allSettled([
        ...
        supabase.from("tokens").select("buyer_user_id").limit(5000),
      ]);
      ...
      if (tokensRes.status === "fulfilled" && tokensRes.value.data) {
        tokensRes.value.data.forEach((t: any) => { if (t.buyer_user_id) tokenCountMap[t.buyer_user_id] = (tokenCountMap[t.buyer_user_id] || 0) + 1; });
      }
      ...
      try {
        ...
        if (authData?.success && Array.isArray(authData.users)) {
          const authMap: Record<string, any> = {};
          authData.users.forEach((u: any) => { authMap[u.id] = u; });
          mapped.forEach((m) => {
            const a = authMap[m.id];
            if (a) {
              m.email = a.email;
              m.last_sign_in_at = a.last_sign_in_at;
              m.phone = a.phone;
            }
          });
        }
  ...
  const adjustCoins = async (action: "add" | "deduct") => {
    ...
    try {
      ...
      toast.success(`${action === "add" ? "+" : "-"}${amount} koin → saldo baru: ${result.new_balance}`);
  ...
  const filtered = useMemo(() => {
    ...
    return users
      .filter(u => !q || (u.username || "").toLowerCase().includes(q) || u.id.toLowerCase().includes(q) || (u.email || "").toLowerCase().includes(q))
      .sort((a, b) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Cari username, email, atau ID..." className="pl-10 bg-background" />
      </div>
                ...
                paged.map(u => (
                  <tr key={u.id} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                    <td className="px-4 py-3">
                      ...
                      {u.email && <p className="text-[10px] text-muted-foreground truncate max-w-[200px]">{u.email}</p>}
                    </td>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-border px-4 py-2">
            ...
            <div className="flex gap-1">
              <Button variant="ghost" size="sm" className="h-7 text-xs" disabled={page === 0} onClick={() => setPage(p => p - 1)}>← Prev</Button>
              <Button variant="ghost" size="sm" className="h-7 text-xs" disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)}>Next →</Button>
            </div>
          </div>
        )}
      </div>
