...
const CoinOrderManager = () => {
  ...
  const fetchOrders = async () => {
    const [ordersRes, pkgRes] = await Promise.all([
      (supabase.from as any)("coin_orders").select("*").order("created_at", { ascending: false }),
      (supabase.from as any)("coin_packages").select("id, name"),
    ]);
  ...
  const deleteOrder = async (id: string) => {
    await (supabase.from as any)("coin_orders").delete().eq("id", id);
    await fetchOrders();
  ...
  const viewProof = async (url: string) => {
    ...
    if (pathMatch) {
      ...
      if (data?.signedUrl) { setPreviewImage(data.signedUrl); return; }
    }
    const { data } = await supabase.storage.from("payment-proofs").createSignedUrl(url, 300);
    ...
    <div className="space-y-6">
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex flex-col items-end gap-2">
                <div className="flex gap-1">
                  <button onClick={() => viewProof(order.payment_proof_url)} className="flex items-center gap-1 rounded-lg bg-secondary px-3 py-1.5 text-xs font-medium text-secondary-foreground hover:bg-secondary/80">
                ...
                {order.status === "pending" && (
                  <div className="flex gap-1">
                    <Button size="sm" variant="default" onClick={() => confirmOrder(order.id)} className="h-7 text-xs"><CheckCircle className="mr-1 h-3 w-3" /> Konfirmasi</Button>
      ...
      <Dialog open={!!previewImage} onOpenChange={() => setPreviewImage(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>Bukti Pembayaran</DialogTitle><DialogDescription>Preview bukti transfer koin</DialogDescription></DialogHeader>
