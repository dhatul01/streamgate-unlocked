...
const CoinOrderManager = () => {
  ...
  const confirmOrder = async (id: string) => {
    ...
    if (error || !(data as any)?.success) {
      toast({ title: "Gagal konfirmasi", description: (data as any)?.error || error?.message, variant: "destructive" });
      return;
    }
    await fetchOrders();
    toast({ title: `Dikonfirmasi! Saldo baru: ${(data as any).new_balance} koin` });
  };
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex flex-col items-end gap-2">
                ...
                {order.status === "pending" && (
                  <div className="flex gap-1">
                    <Button size="sm" variant="default" onClick={() => confirmOrder(order.id)} className="h-7 text-xs">
                      <CheckCircle className="mr-1 h-3 w-3" /> Konfirmasi
                    </Button>
                    <Button size="sm" variant="destructive" onClick={() => rejectOrder(order.id)} className="h-7 text-xs">
      ...
      <Dialog open={!!previewImage} onOpenChange={() => setPreviewImage(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Preview bukti transfer koin</DialogDescription>
          </DialogHeader>
