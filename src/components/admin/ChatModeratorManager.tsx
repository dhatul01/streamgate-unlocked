...
const ChatModeratorManager = () => {
  ...
  const fetchMods = async () => {
    ...
    if (data) setMods(data as ChatMod[]);
  };
  ...
  const addMod = async (e: React.FormEvent) => {
    ...
    setAdding(true);
    const { error } = await supabase.from("chat_moderators").insert({ username: trimmed });
    setAdding(false);
    if (error) {
  ...
  const removeMod = async (id: string, uname: string) => {
    const { error } = await supabase.from("chat_moderators").delete().eq("id", id);
    if (!error) {
  ...
  return (
    <div className="space-y-4 rounded-xl border border-border bg-card p-6">
      ...
      <p className="text-[11px] text-muted-foreground">
        Tambahkan username viewer sebagai moderator chat. Moderator dapat menghapus pesan dan memblokir pengguna di live chat.
      </p>
          ...
          {mods.map((m) => (
            <div key={m.id} className="flex items-center justify-between rounded-lg border border-border bg-background px-3 py-2">
              ...
              <Button
                ...
                onClick={() => removeMod(m.id, m.username)}
              >
