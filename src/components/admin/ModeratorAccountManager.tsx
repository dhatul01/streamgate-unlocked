...
const ModeratorAccountManager = () => {
  ...
  const createModerator = async (e: React.FormEvent) => {
    ...
    const res = await fetch(
      `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/manage-moderator-account`,
      {
        method: "POST",
        headers: {
    ...
    if (result.error) {
      ...
      toast({ title: "Berhasil", description: `Akun moderator ${username} berhasil dibuat.` });
  ...
  const deleteModerator = async (userId: string, modUsername: string) => {
    ...
    if (result.error) {
      ...
      toast({ title: "Dihapus", description: `Akun ${modUsername} berhasil dihapus.` });
      fetchModerators();
    }
    setDeletingId(null);
  };
                ...
                <TableRow key={mod.id}>
                  ...
                  <TableCell>
                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
                      mod.is_active ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500"
                    }`}>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Akun <strong>{mod.username}</strong> akan dihapus permanen beserta seluruh datanya. Tindakan ini tidak bisa dibatalkan.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          <AlertDialogCancel>Batal</AlertDialogCancel>
                          <AlertDialogAction
                            onClick={() => deleteModerator(mod.user_id, mod.username)}
