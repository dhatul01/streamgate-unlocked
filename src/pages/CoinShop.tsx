...
const CoinShop = () => {
  ...
  const handleUploadProof = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    formData.append("show_id", "coin-purchase"); // dummy for edge fn
  ...
  const handleRedeem = async (showId: string) => {
    ...
    if (error || !(data as any)?.success) {
      toast({
        title: "Gagal menukar koin",
        description: (data as any)?.error || error?.message || "Coba lagi",
        variant: "destructive",
      });
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-2xl px-4 py-6">
        ...
        {tab === "buy" && (
          <div className="space-y-3">
            ...
            {packages.length === 0 && (
              <p className="py-8 text-center text-sm text-muted-foreground">Belum ada paket tersedia</p>
            )}
              ...
              {packages.map((pkg) => (
                ...
                >
                  <div className="mb-3 flex items-center gap-2">
                    ...
                    <span className="text-lg font-bold text-foreground">{pkg.coin_amount} Koin</span>
                  </div>
                  ...
                  <Button size="sm" className="w-full" onClick={() => handleBuyPackage(pkg)}>
                    Beli Sekarang
                  </Button>
                </motion.div>
              ))}
        ...
        {tab === "redeem" && (
          <div className="space-y-3">
            ...
            {shows.length === 0 && (
              <p className="py-8 text-center text-sm text-muted-foreground">Tidak ada show yang tersedia</p>
            )}
            {shows.map((show) => (
              <div key={show.id} className="flex items-center justify-between rounded-xl border border-border bg-card p-4">
                ...
                <Button
                  size="sm"
                  disabled={balance < show.coin_price || redeemingShow === show.id}
                  onClick={() => handleRedeem(show.id)}
                >
        ...
        <button
          onClick={() => navigate("/")}
          className="mt-8 flex w-full items-center justify-center gap-2 text-sm text-muted-foreground hover:text-foreground"
        >
      ...
      <Dialog open={!!selectedPkg} onOpenChange={() => setSelectedPkg(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Beli {selectedPkg?.coin_amount} Koin</DialogTitle>
            <DialogDescription>{formatPrice(selectedPkg?.price || 0)}</DialogDescription>
          </DialogHeader>
          ...
          {purchaseStep === "qris" && selectedPkg?.qris_image_url && (
            <div className="space-y-3">
              ...
              <p className="text-center text-xs text-muted-foreground">Scan QRIS di atas, lalu upload bukti bayar</p>
              <Button className="w-full" onClick={() => setPurchaseStep("upload")}>
            ...
            <div className="space-y-3">
              <label className="flex cursor-pointer flex-col items-center gap-2 rounded-xl border-2 border-dashed border-border p-8 hover:border-primary">
                ...
                <span className="text-sm text-muted-foreground">
                  {uploading ? "Mengupload..." : "Tap untuk upload bukti bayar"}
                </span>
          ...
          {purchaseStep === "done" && (
            <div className="space-y-3 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Koin akan ditambahkan setelah admin mengkonfirmasi pembayaran.
              </p>
              <Button className="w-full" onClick={() => setSelectedPkg(null)}>
                Tutup
              </Button>
            </div>
          )}
      ...
      <Dialog open={!!redeemResult} onOpenChange={() => setRedeemResult(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>Gunakan token ini untuk menonton</DialogDescription>
          </DialogHeader>
          {redeemResult && (
            <div className="space-y-4 text-center">
              <div className="rounded-lg bg-secondary p-4">
                <p className="font-mono text-lg font-bold text-primary">{redeemResult.token_code}</p>
              </div>
              <Button
                className="w-full gap-2"
                onClick={() => copyToken(redeemResult.token_code)}
              >
                <Copy className="h-4 w-4" /> Salin Link Nonton
              </Button>
              <p className="text-xs text-muted-foreground">
                Sisa saldo: <span className="font-bold text-warning">{redeemResult.remaining_balance} koin</span>
              </p>
            </div>
          )}
        </DialogContent>
