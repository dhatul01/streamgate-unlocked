...
const ReplayPage = () => {
  ...
  const handleReplayRedeem = async () => {
    ...
    setRedeeming(true);
    const { data, error } = await supabase.rpc("redeem_coins_for_replay", { _show_id: replayTarget.id });
    setRedeeming(false);
      ...
      <div className="mx-auto max-w-6xl px-4 pt-20 pb-12">
        ...
        <div className="relative mx-auto mb-8 max-w-md">
          ...
          <Input
            ...
            placeholder="Cari nama show atau tanggal..."
        ...
        {coinUser && (
          <div className="mx-auto mb-6 flex max-w-md items-center justify-between rounded-xl border border-border bg-card p-3">
            ...
            <span className="flex items-center gap-1.5 font-bold text-warning">
              <Coins className="h-4 w-4" /> {coinBalance} Koin
            </span>
          </div>
        )}
                  ...
                  <div className="space-y-2.5 p-4">
                    ...
                    {hasPassword ? (
                      ...
                      <button
                        onClick={() => {
                          if (!coinUser) {
                            toast({ title: "Login terlebih dahulu", description: "Silakan login di /auth untuk membeli replay.", variant: "destructive" });
                            return;
                          }
                          setReplayTarget(show);
                          setReplayResult(null);
                        }}
                        className="flex w-full items-center justify-center gap-2 rounded-xl bg-warning py-3 font-semibold text-warning-foreground transition-all hover:bg-warning/90"
                      >
          ...
          {!replayResult ? (
            <div className="space-y-4">
              <div className="rounded-xl border border-border bg-secondary/50 p-4 space-y-2">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-bold text-warning">{replayTarget?.replay_coin_price} Koin</span>
                </div>
                <div className="flex items-center justify-between text-sm border-t border-border pt-2">
                  <span className="text-muted-foreground">Saldo Anda</span>
                  <span className={`font-bold ${coinBalance >= (replayTarget?.replay_coin_price || 0) ? "text-success" : "text-destructive"}`}>
                    {coinBalance} Koin
                  </span>
                </div>
              </div>
            ...
            <div className="space-y-4 text-center">
              ...
              <div className="rounded-lg border border-warning/30 bg-warning/10 p-4">
                ...
                <p className="font-mono text-2xl font-bold text-warning">{replayResult.replay_password}</p>
              </div>
              <Button
              ...
              {!replayCopied && (
                <p className="text-xs text-destructive">⚠️ Salin sandi terlebih dahulu</p>
              )}
              <p className="text-xs text-muted-foreground">Sisa saldo: <span className="font-bold text-warning">{replayResult.remaining_balance} koin</span></p>
            </div>
          )}
        </DialogContent>
      </Dialog>
