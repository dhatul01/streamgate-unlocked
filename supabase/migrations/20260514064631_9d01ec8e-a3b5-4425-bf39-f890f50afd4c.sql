-- Drop old signature, add _show_id parameter with show-aware enforcement
DROP FUNCTION IF EXISTS public.bot_create_token(text, text, integer, boolean);
...
BEGIN
  ...
  -- Generate guaranteed-unique code (UNIQUE constraint on tokens.code is the final barrier)
  LOOP
  ...
  -- Always FRESH: locked_fingerprint NULL, no buyer_user_id, status active
  ...
  IF NOT _is_admin THEN
    ...
    VALUES (_reseller.id, 'create_token_bot', _token.id,
      jsonb_build_object(
        'code', _token.code, 'via', 'whatsapp_bot', 'phone', _actor_phone,
  ...
  RETURN json_build_object(
    ...
    'show_title', CASE WHEN _show_id IS NOT NULL THEN _show.title ELSE NULL END,
...
REVOKE EXECUTE ON FUNCTION public.bot_create_token(text, text, integer, boolean, uuid) FROM anon, authenticated, public;